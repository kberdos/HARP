...
We then extended the comparison to include DOTE and TEAL under the same dynamic Abilene held-out slice and the same MLU-resilience evaluator. These rows should be read as HARP-data adapters rather than exact reproductions of the original papers' native experimental setups: the DOTE row uses the MAXUTIL multilayer perceptron with input and output dimensions changed for the 12-node, 4-path Abilene samples, while the TEAL row uses the FlowGNN actor structure with each sample's final topology graph. Both adapters were trained on samples 0--800 and evaluated on samples 800--1000, matching the HARP split.
...
DOTE adapter & 1.1545 & 2.0562 & 2.5960 & 4.6179 \\
TEAL adapter & 2.7472 & 4.7729 & 5.2167 & 10.9889 \\
\hline
\end{tabular}
\end{center}
